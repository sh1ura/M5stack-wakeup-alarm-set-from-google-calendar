...
String calendar[] = { // iCalendar address of google calendar / google カレンダーの iCal のアドレス
  "https://calendar.google.com/calendar/ical/xxxxxxxxxxxxxxxxxxxxxxxxxx%40group.calendar.google.com/public/basic.ics", // alarm time
  "https://calendar.google.com/calendar/ical/yyyyyyyyyyyyyyyyyyyyyyyyyy%40group.calendar.google.com/public/basic.ics", // event calendar 1
  "https://calendar.google.com/calendar/ical/zzzzzzzzzzzzzzzzzzzzzzzzzz%40group.calendar.google.com/public/basic.ics"  // event calendar 2
};

String calName[] = { // short name of each calendar shown on display / それぞれのカレンダーの略称（表示用）
...
// それぞれのカレンダーについて，イベントの何分前に目覚ましを鳴らすか
// time between alarm and event, in minutes
int margin[] = {0, 120, 120};
...
void loop(){
  ...
  for(int calNo = 0; calNo < CALNUM; calNo++) {
    int getStat = getSchedStart(calendar[calNo]);
    if(getStat == false && alarmSet == false) { // if alarm is not set yet / アラーム未設定
      failCount++;
    ...
    failCount = 0;
    M5.Lcd.print(calName[calNo]);
    while(1) { // access iCal / iCal 取得
      String line = getSchedLine();
      if(line.length() == 0) {
        break;
      }
...
//      Serial.println("[" + line + "]");
      String dt = line.substring(8); // time string in GMT / 時刻の文字列（GMT）
      ...
      if(t.tm_year == curtime.tm_year && t.tm_mon == curtime.tm_mon && t.tm_mday == curtime.tm_mday) {
        ...
        time_t et = mktime(&t) - margin[calNo] * 60; // calc alarm time / アラームを鳴らす時刻を計算
